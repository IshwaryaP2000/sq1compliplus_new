...
const MyPolicy = () => {
  ...
  const getMyPolicies = async () => {
    try {
      setIsLoading(true);
      const response = await getApi("/policy/all/my-policy");
  ...
  // Handle policy retirement (for active policies)
  ...
  // Handle policy review submission (for active policies)
  ...
  // Handle policy change (for expired policies)
  ...
  // Save policy content with option to approve (for template_data and pending approval)
  ...
  // Approve policy (for pending approval)
  ...
  // Recommission policy (for retired policies)
  const handleRecommissionPolicy = async () => {
    try {
      setIsApproving(true);
      const fieldName = `expiry_date_${selectedPolicy?.id}`;
      const payload = {
      ...
      await postApi(`/policy/approve/${selectedPolicy?.id}`, payload);
      await getMyPolicies();
      setShowRecommissionModal(false);
    } catch (error) {
      console.error("Error recommisioning policy:", error);
    } finally {
      setIsApproving(false);
    }
  };

  // Extend policy (for expired policies)
  const handleExtendPolicy = async () => {
    try {
      ...
      console.error("Error extending policy:", error);
    } finally {
      setIsApproving(false);
    }
  };

  // Show approve modal with default review date (1 year from now)
  ...
  // Function to render status badges for all policy statuses
    ...
    return (
      <div className="row">
        {policies.map((policy) => {
          ...
          return (
            <div className="col-md-6 mb-1" key={policy.id}>
              <div
              ...
              >
                <div className="card-body p-0">
                  ...
                  <div className="d-flex justify-content-between mb-3">
                    <div className="policy-details">
                      <p className="mb-0 fs-14 text-grid-light mb-2">
                        Policy Title
                      </p>
                      <h5
                        dangerouslySetInnerHTML={{
  ...
  return (
    <>
      ...
      {Object.entries(filteredData?.all_policies || {}).map(
        ([category, policies]) => (
          <div key={category} className="float-left mb-4">
            <div className="d-flex justify-content-between mb-3 flex-wrap">
              <h5>
      ...
      >
        ...
        <Modal.Footer className="d-flex justify-content-center">
          ...
          >
            {isSubmittingReview ? (
              <>
                <ButtonWithLoader name="Submiting..." />
              </>
            ) : (
              "Submit"
            )}
      ...
      >
        ...
        <Modal.Footer>
          ...
          <Button
            variant="success"
            onClick={handleRecommissionPolicy}
            disabled={isApproving}
          >
            {isApproving ? (
              <>
